...
ConstExprFunctionState::computeWellKnownCallResult(ApplyInst *apply,
  ...
  case WellKnownFunction::ArrayCopyToNewBuffer: {
    ...
    for (auto *use : arrayAddress->getUses()) {
      ...
      if (!buffer || !buffer->getReferencedFunctionOrNull()->hasSemanticsAttr(
                         semantics::CONTIGUOUS_ARRAY_BUFFER_INIT))
        continue;
      ...
      auto one = builder.createIntegerLiteral(dummyLoc, sizeType, 1);
      auto report = builder.createIntegerLiteral(dummyLoc, boolType, -1);
      auto addReturnType = TupleType::get({TupleTypeElt(sizeType.getASTType()),
                                           TupleTypeElt(boolType.getASTType())},
                                          builder.getASTContext());
      auto addReturnSILType =
          SILType::getPrimitiveObjectType(CanType(addReturnType));
      auto newSizeRes = builder.createBuiltinBinaryFunction(
          SILDebugLocation().getLocation(), "sadd_with_overflow", sizeType,
          addReturnSILType, {size, one, report});
      ...
      auto newSizeInt =
          builder.createStruct(dummyLoc, sizeInt->getType(), {newSize});
