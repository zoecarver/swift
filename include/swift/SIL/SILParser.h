...
public:
  ...
  SILValue getLocalValue(UnresolvedValueName Name, SILType Type, SILLocation L,
                         SILBuilder &B);
  SILValue getLocalValue(SILParserOperand name, SILBuilder &builder,
...
class ReadSIL : public ReadSILBase, public Parser {
  ...
  bool readSingleOperand(SILParserOperands &instOperands);
...
public:
  ...
  void readCopyValueInst(SILParserResult &out);
...
class CheckSIL : public CheckSILBase {
  ...
  InFlightDiagnostic diagnose(SourceLoc loc, Diag<DiagArgs...> diagID,
                              Args&&... args);
public:
  CheckSIL(SILModule &module, SILValueVault &vault)
    : module(module), vault(vault) {}

  // Entry point for the visitor.
  bool check(SILParserResult);

  bool checkCopyValueInst(SILParserResult);
};
...
class EmitSIL : public EmitSILBase {
  ...
  // Convenience method for getting and setting values using vault.
  void setLocalValue(ValueBase *Value, StringRef Name, SourceLoc NameLoc);
  SILValue getLocalValue(SILParserOperand name, SILBuilder &builder,
                         SILLocation loc = RegularLocation::getAutoGeneratedLocation());
...
public:
  EmitSIL(SILModule &module, SILValueVault &vault)
    : module(module), vault(vault) {}
  ...
  SILInstruction *emitCopyValueInst(SILBuilder&, SILParserResult);
};
